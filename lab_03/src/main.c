...
void print_array(const byte_t *const array, const size_t size)
{
...
void sub_bytes(byte_t *block, byte_t table[16][16], const size_t size)
{
    for (size_t i = 0; i < size; ++i)
    {
...
void copy_block(byte_t *dst, const byte_t *const src, const size_t size)
{
    for (size_t i = 0; i < size; ++i)
    {
        dst[i] = src[i];
...
void transpose_block(byte_t *block, const size_t size)
{
...
void shift_rows_left(byte_t *block)
{
...
void shift_rows_right(byte_t *block)
{
    transpose_block(block, BLOCK_SIZE);
    // second row
    byte_t tmp = block[7];
...
void mix_columns(byte_t *block, byte_t *result)
{
    transpose_block(block, BLOCK_SIZE);
...
void inv_mix_columns(byte_t *block, byte_t *result)
{
    transpose_block(block, BLOCK_SIZE);
    for (size_t i = 0, j = 0; j < 4; ++i, ++j)
    {
...
void rotate_key(byte_t *key, byte_t *result)
{
    result[0] = key[13];
    result[1] = key[14];
    result[2] = key[15];
...
void add_round_const(byte_t *key, const size_t round)
{
    key[0] = key[0] ^ round_constants[round];
}

void generate_keys(const byte_t *const key)
{
    byte_t tmp[BLOCK_SIZE];
...
int main(int argc, char **argv)
{
    ...
    if (strcmp(argv[1], "-e") == 0)
    {
        ...
        while (len != 0)
        {
            ...
            encrypt(plain_block, cipher_block, key);
    ...
    else if (strcmp(argv[1], "-d") == 0)
    {
        ...
        while (len != 0)
        {
            ...
            decrypt(plain_block, cipher_block, key);
            fwrite(cipher_block, sizeof(byte_t), BLOCK_SIZE, f_out);
